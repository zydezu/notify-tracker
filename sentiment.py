...
def score_message(text: str) -> int:
    ...
    """
    tokens = cast(list[str], WORD_RE.findall(text.lower()))
    score = 0
...
def analyze(app_filter: str | None = None) -> None:
    ...
    if app_filter:
        ...
        raw_rows = conn.execute(
            "SELECT message, received_at FROM notifications"
        ).fetchall()
    rows = cast(list[tuple[str, str]], raw_rows)
